...
- **Advisor tool calling (2026-09-05)**: advisor models may call the tools visible to the current DSH session before answering (`read`/`grep`/`glob`/`web_search`/`web_fetch`…) through the official `ctx.tools` pipeline — scoped dispatch with the initiating agent, monotonic guards and pre/post policies included. The loop caps at 4 tool rounds, then forces a textual answer; every invocation is narrated into the 💭 thinking panel (`🔧 模型请求调用工具…` + result preview). Scope: `discussion.advisorTools` = `readonly` (default read-only whitelist) | `all` (every session-visible tool, incl. writable ones) | `off`. Constraint: `ctx.llm` (dsh-llm 0.1.2-rc.1) has no tool-calling support, so tool use requires the direct-http OpenAI/Anthropic channel — give that advisor `baseURL`/`apiKey` (a DSH-provider-only advisor answers without tools).
...
- **Boundary matrix (advisor audit item 3) closed**: ① a resume stopped BETWEEN rounds no longer asks the follow-up twice — a fresh round only generates a deep-question when its main-model question count is below the round number (the pushed follow-up is already in the message log); ② a stop DURING driver follow-up generation now propagates as cancellation instead of pushing the static fallback as if the question had been asked, so resume regenerates exactly one follow-up. Boundary tests: `stop BETWEEN rounds` (no duplicate) and `stop during generation` (exactly one question) plus driver abort-propagation / real-failure-fallback tests.
